...
#[test]
fn model_only_answers_defaults_to_false_with_shipped_config() {
    let repo_root = Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap();
    let temp_dir =
        std::env::temp_dir().join(format!("lancet-cfg-test-mo-def-{}", uuid::Uuid::new_v4()));
    let lancedb_dir = temp_dir.join("lancedb");
...
#[test]
fn model_only_answers_recognizes_true_and_false_env_overrides() {
    let repo_root = Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap();
    let config_dir = repo_root.join("config");
    for (env_val, _expected) in [("true", true), ("1", true), ("false", false), ("0", false)] {
        let temp_dir =
            std::env::temp_dir().join(format!("lancet-cfg-test-mo-val-{}", uuid::Uuid::new_v4()));
        let lancedb_dir = temp_dir.join("lancedb");
        let cwd_dir = temp_dir.join("empty_cwd");
        fs::create_dir_all(&cwd_dir).unwrap();
        ...
        let env_vars = [
            ...
            ("LANCET_ENGINE__GRPC_ADDR", "127.0.0.1:0"),
            ("LANCET_ENGINE__LANCEDB_PATH", lancedb_path.as_str()),
            ("LANCET_ENGINE__WORKFLOW__ALLOW_MODEL_ONLY_ANSWERS", env_val),
            ("OPENROUTER_API_KEY", "test-key"),
        ];
...
#[test]
fn model_only_answers_invalid_env_fails_closed() {
    let repo_root = Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap();
    let config_dir = repo_root.join("config");
    let temp_dir =
        std::env::temp_dir().join(format!("lancet-cfg-test-mo-inv-{}", uuid::Uuid::new_v4()));
    let lancedb_dir = temp_dir.join("lancedb");
    let cwd_dir = temp_dir.join("empty_cwd");
    fs::create_dir_all(&cwd_dir).unwrap();
    fs::create_dir_all(&lancedb_dir).unwrap();
    ...
    for invalid_val in ["invalid", "yes", "2", "TRUE_EXTRA"] {
        ...
        let err_msg = match result {
            Ok((child, _, _)) => {
                cleanup_child(child);
                panic!("engine must reject invalid LANCET_ENGINE__WORKFLOW__ALLOW_MODEL_ONLY_ANSWERS value {invalid_val:?}")
            }
            Err(error) => error,
        };
        assert!(
        ...
        assert!(
            err_msg.contains("LANCET_ENGINE__WORKFLOW__ALLOW_MODEL_ONLY_ANSWERS"),
...
fn model_only_answers_empty_or_whitespace_env_treated_as_absent() {
    ...
    for empty_val in ["", "   \t\n  "] {
        let temp_dir =
            std::env::temp_dir().join(format!("lancet-cfg-test-mo-emp-{}", uuid::Uuid::new_v4()));
        let lancedb_dir = temp_dir.join("lancedb");
        let cwd_dir = temp_dir.join("empty_cwd");
        fs::create_dir_all(&cwd_dir).unwrap();
        fs::create_dir_all(&lancedb_dir).unwrap();
