...
async fn openrouter_node_optin_empty_evidence_retrieval_basis_still_yields_model_only() {
    ...
    let generate_node = workflow::nodes::GenerateAnswerNode::new(Some(generator)).with_settings(
...
#[tokio::test]
async fn openrouter_node_standalone_near_miss_marker_is_repaired() {
    use serde_json::json;
    use std::io::{Read, Write};
    use std::net::TcpListener;
    use std::sync::atomic::{AtomicUsize, Ordering};
    ...
    let req = test_query_request(
        "Repair near-miss marker query",
        "00000000-0000-4000-8000-000000000089",
    );
    let mut ctx = WorkflowContext::new(
        "00000000-0000-4000-8000-000000000089".into(),
        "trace-sc5-repair".into(),
        &req,
    );
    ctx.evidence_blocks = vec![crate::prompt::EvidenceBlock {
        id: "[7]".to_string(),
        chunk_id: "chk-7".to_string(),
        document_id: "doc-7".to_string(),
        chunk_index: 0,
        title: Some("Title 7".to_string()),
        section_path: Some("Section 7".to_string()),
        content_type: Some("text/plain".to_string()),
        provenance: "provenance".to_string(),
        text: "Evidence for chunk 7".to_string(),
        score: 0.95,
        rank: 1,
        suspicious: false,
    }];
    ...
    assert!(ctx
        .notices
        .iter()
        .any(|n| n.typed_code == v1::NoticeCode::CitationRepaired as i32));

    server_handle.join().expect("server join");
}

#[tokio::test]
async fn openrouter_node_strict_visible_unresolvable_marker_is_dropped() {
    use serde_json::json;
    use std::io::{Read, Write};
    use std::net::TcpListener;
    use std::sync::atomic::{AtomicUsize, Ordering};
    ...
    let req = test_query_request(
        "Drop unresolvable marker query",
        "00000000-0000-4000-8000-000000000090",
    );
    let mut ctx = WorkflowContext::new(
        "00000000-0000-4000-8000-000000000090".into(),
        "trace-sc5-drop".into(),
        &req,
    );
    ctx.evidence_blocks = vec![crate::prompt::EvidenceBlock {
    ...
    assert!(!ctx.answer.contains("[9]"));
    assert!(ctx
        .notices
        .iter()
        .any(|n| n.typed_code == v1::NoticeCode::CitationDropped as i32));

    server_handle.join().expect("server join");
}

#[tokio::test]
async fn openrouter_node_total_citation_loss_downgrades_basis_to_model_only() {
    use serde_json::json;
    use std::io::{Read, Write};
    use std::net::TcpListener;
    use std::sync::atomic::{AtomicUsize, Ordering};
...
#[tokio::test]
async fn openrouter_node_model_only_flag_off_stays_fail_closed() {
    use serde_json::json;
    use std::io::{Read, Write};
    use std::net::TcpListener;
    use std::sync::atomic::{AtomicUsize, Ordering};
    ...
    let server_handle = std::thread::spawn(move || {
        ...
        let start = std::time::Instant::now();
        let mut conn_count = 0;
        while conn_count < 1 && start.elapsed() < Duration::from_secs(5) {
            match listener.accept() {
                Ok((mut stream, _)) => {
                    conn_count += 1;
                    stream.set_nonblocking(false).unwrap();
    ...
    let mut req = test_query_request(
        "Flag off empty evidence query",
        "00000000-0000-4000-8000-000000000092",
    );
    req.allow_model_only = Some(false);
    let mut ctx = WorkflowContext::new(
        "00000000-0000-4000-8000-000000000092".into(),
        "trace-sc3-flag-off".into(),
        &req,
    );
    ctx.evidence_blocks = vec![];
    ...
    assert_eq!(err.kind, v1::NodeErrorKind::LlmGenerationFailed);
    assert!(
        err.message.contains("prompt assembly failed"),
        "error message should contain 'prompt assembly failed', got: {}",
        err.message
    );
