...
- Native build deps for the engine: `build-essential`, `pkg-config`, `libssl-dev` (transitive `openssl-sys` from LanceDB's object-store stack), and `protobuf-compiler` (`protoc`, needed by a dependency `build.rs`). The engine's own protobufs are pre-generated under `engine/src/pb`, so `buf` is not required to build.
