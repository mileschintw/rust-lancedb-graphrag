...
| RESEARCH | Isolated LanceDB fixtures, fake embedding/generator seams, startup readiness proof, provider-independent Go-to-generated-gRPC-to-Rust smoke, and Rust/Go test commands. | 03-01, 03-02, 03-03, 03-04, 03-05 | COVERED | Unit fixtures and fake Generator are introduced at their owning handoffs; Plan 03-04 adds the embedding endpoint seam; Plan 03-05 supplies the reusable real seed, three deterministic provider mocks, post-log Ping readiness, clean child environments, direct-binary process-tree cleanup, LanceDB release proof, and the cross-runtime assertion. |
...
| D-37 | Prompt-injection text remains marked evidence and is flagged. | 03-02 | COVERED by the focused valid-path assertion `suspicious_evidence_remains_marked_unexecuted`; the source remains evidence and no fallback branch is added. |
| D-38 | Corpus conflicts are disclosed and classified as mixed. | 03-02 | COVERED by the focused valid-path assertion `corpus_conflict_returns_mixed_basis_with_disclosure`; this exercises structured mixed-basis disclosure without adding degraded/model-only orchestration. |
