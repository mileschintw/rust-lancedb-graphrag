...
**Researched:** 2026-08-06 (Phase 04 spike research; adopted as Phase 04.1's research 2026-08-07)
...
> **Provenance:** This document is `04-RESEARCH.md` carried over from Phase 04 (the `lance-graph`/`lancedb`
> compatibility spike) at the user's explicit direction during the Phase 04.1 `--reviews` replan —
> Phase 04 is 04.1's origin phase and this research was authored as forward-looking context for it (see
> the original scope note below, preserved unchanged). Bring-over is mechanical (filename, this header);
> the technical content — Standard Stack, Architecture Patterns, Pitfalls, Code Examples, Assumptions,
> Open Questions, Security Domain — is unchanged from Phase 04 and remains accurate: it describes
> empirically-verified facts about `lance-graph` 0.5.4, not phase-scoped predictions that could go stale.
> The `## Validation Architecture` section below IS phase-scoped and forward-looking (task IDs, file
> "exists" status); **it has been superseded by `04.1-VALIDATION.md`**, which was updated in this
> replan against the actual checked-in code — treat that file, not the section below, as current.
>
> **Original scope note (Phase 04, preserved as written):** Per the orchestrator's narrowing, this
> research (and the phase it supports) covers **only** the `lance-graph`/`lancedb` integration spike —
> not the full extraction/storage/query-traversal implementation described in Phase 4's original success
> criteria. Those success criteria, and the `## Phase Requirements` mapping below, are preserved as
> forward-looking context for the follow-up phase (04.1) that this spike unblocks.
...
> **⚠ Superseded by `04.1-VALIDATION.md`.** This section is preserved verbatim as it was written
> during Phase 04 (forward-referencing task IDs like `04.1-01-01` and a `❌ Wave 0 (04.1)` "file
> exists" column that predates this repository having a Phase 04.1 at all). Phase 04.1's own
> `04.1-VALIDATION.md` has since been verified against the actual checked-in `engine/src/graph/`
> code (5/5 tests passing under the `graph-spike` feature as of 2026-08-07) and extended to cover
> Waves 2-5. Read `04.1-VALIDATION.md` for the current validation contract; the tables below are
> historical record of what the spike anticipated, not a live source of truth.
