...
## Open Questions (RESOLVED — see `/gsd-plan-phase 6.2` disposition below each)
...
1. **How will the current wire `WorkflowEvent.trace_id`/checkpoint `trace_id` field map to D-27’s retained `correlation_id` while OTel SpanContext owns telemetry trace identity?**
   ...
   - **RESOLVED:** `06.2-01-PLAN.md` makes this an explicit `must_haves` truth and prohibition (correlation_id stays the shipped wire value; the OTel trace ID is span-attribute-only), with a dedicated test.
...
2. **What exact Go context-aware logger seam should be used for existing zap calls and background checkpoint logs?**
   ...
   - **RESOLVED:** `06.2-01-PLAN.md` proves exactly one request-scoped log site end to end (RESEARCH A4); `06.2-06-PLAN.md` migrates the remaining call sites across both runtimes.
...
3. **Does Loki native OTLP expose the log record trace ID in a field usable by Grafana’s Jaeger trace-to-log link?**
   ...
   - **RESOLVED:** `06.2-03-PLAN.md` carries this as a dedicated `<human-check>` Compose smoke assertion (trace/log pair emitted, Loki queried, Jaeger trace-to-log link followed).
...
4. **Where should the typed dashboard generator’s Go module live?**
   ...
   - **RESOLVED:** `06.2-03-PLAN.md` picks and records one explicit module layout for the dashboard generator before writing the regeneration command.
...
5. **Should the Collector and backend pins be candidate pins or current release pins?**
   ...
   - **RESOLVED:** `06.2-01-PLAN.md` pins Collector `0.159.0`, keeps Jaeger at the existing `2.19.0`, matching the recommendation.
