...
- **D-17:** Per-node timeouts use the plan doc's example values as defaults: `ReformulateQuery` 5s, `HybridRetrieval` 10s, `GraphExtraction` 15s, `PromptAssembly` 2s, `LLMGeneration` 30s (the last already matches Phase 3 D-30's existing configurable generation timeout — no conflict, just formalized as one node's timeout). All configurable via the existing TOML+env override convention (Phase 2 D-26–D-30).
