...
- [x] 06.2-10-PLAN.md — Close G-06.2-1: Grafana correlation — remove unresolvable `tracesToLogsV2` tags, Loki `matcherType: label` / `matcherRegex: trace_id`, SC1 continuity in the human check (D-34, D-40, D-27, D-28 / OBS-01).
...
- [x] 06.2-11-PLAN.md — Close G-06.2-2: Operations dashboard Panel 1 p95 `histogram_quantile` over `_bucket`, `Unit("ms")` on both duration panels, regenerated JSON (D-35, D-40 / OBS-01).

**Wave 10** *(gap closure — G-06.2-4; depends on 06.2-01, 06.2-09, 06.2-11, same shared-file serialization reason as Wave 9)*

- [x] 06.2-12-PLAN.md — Close G-06.2-4: windowed OTel diagnostics bound (at most one diagnostic per 5 minutes, re-arm) in Go (`otel.SetErrorHandler`) and Rust (`tracing-subscriber` layer filter) (D-38 / OBS-01).
...
- [x] 06.3-03-PLAN.md — Corpus as a config entry (both corpora through one loader), the committed seeded 500-question sample and attributed document subset, and the full deterministic metric set on one declared matching rule computed over the retrieved-chunk list (D-44, D-50, D-53, D-55, D-59).

**Wave 4** *(blocked on 06.3-01, 06.3-03)*

- [x] 06.3-04-PLAN.md — Eval store isolation via the existing Atlas migrations on a separate schema plus an isolated LanceDB path, seeding through the real ingestion path with the seed-time document map, and the fail-fast preflight (D-55, D-56, D-57, D-58, D-64, D-84).

**Wave 5** *(blocked on 06.3-01, 06.3-03, 06.3-04)*

- [x] 06.3-05-PLAN.md — Resumable append-only journal, both graph-ablation arms against one running engine, the ablation dimension with its notice-provenance check, and the offline deterministic scorer (D-46, D-47, D-50, D-53).

**Wave 6** *(blocked on 06.3-01, 06.3-03, 06.3-04, 06.3-05)*

- [x] 06.3-06-PLAN.md — Pinned, temperature-zero, distinct-family LLM judge with an auditable plain-text cache, groundedness and faithfulness behind a seeded judged slice, and the human calibration slice (D-50, D-52, D-53, D-62).

**Wave 7** *(blocked on all preceding plans)*

- [x] 06.3-07-PLAN.md — Full run metadata with publication preconditions, per-corpus Markdown and JSON reporting with no aggregate, the regenerated schema, and the committed recorded run (D-54, D-60, D-61).
...
- [x] 06.3.1-01-PLAN.md — Tracer: publish notice code 19, regenerate both proto trees, emit the retrieval-failure notice from the runner (the only place a node timeout is observed), and pin it end to end through the Go gateway (SC-1, SC-7; D-28, D-52, D-50)
- [x] 06.3.1-02-PLAN.md — Fail-closed LanceDB table access: propagate every `open_table` error except the missing-table condition, at the shared function backing all six accessors, with a regression test (SC-5; D-24, D-25)
- [x] 06.3.1-03-PLAN.md — Graph-path legibility: retain `graph_traversal` spans regardless of the configured sampling ratio, and document timeout-budget nesting in both operator TOML files without changing a value (SC-6, SC-8; D-08, D-17)

**Wave 2** *(blocked on Wave 1 completion — 06.3.1-01, -02, -03)*

- [x] 06.3.1-04-PLAN.md — Provenance on failure and graph influence: seed the retrieval snapshot before the first await so a killed node still reports its index generation, count graph facts that reached the assembled prompt, surface both through the SSE contract, and close the phase test-count invariants (SC-2, SC-3, SC-4, SC-7; D-33, D-06, D-29, D-30, D-52)
...
- **`is_usable()` is the single denominator predicate for the whole phase.** Declared in plan 01, consumed by 02, 04 and 05. Infrastructure-failed records leave *every* quality mean — retrieval and answer alike. The old asymmetry (retrieval excluded them, answer scored them as zeros) is what produced the headline `answer_exact_match = 0.000`.
...
- **The measurement pass is therefore two-armed, with each question's arms dispatched adjacently.** A single-armed pass would leave two of the six budgets with no derivation input. `--sample-size` counts **questions**; the drive issues two queries per question, so SC-2's "several hundred queries" is the record count, not the question count.
- **Measurement records carry `ordinal`/`segment`/warm-up on their own record type.** `RunRecord` is `extra="forbid"`, so those fields cannot be attached to it; `measure.py` defines a `MeasurementRecord` extending it and every reader in the phase parses with that type. The benchmark journal schema is untouched.
...
**Plans:** 10/10 plans complete — plans 01-09 executed (including 2 gap-closure plans closed 2026-09-12), closing `06.3.4-VERIFICATION.md` gap 0, `06.3.4-REVIEW.md` CR-01/WR-01/WR-05/IN-01/IN-02/IN-03/WR-03, and `06.3.4-SECURITY.md` T-06.3.4-19/24/43. Plan 10 (a third gap-closure round, executed 2026-09-13) closed `06.3.4-SECURITY.md`'s two remaining blocking threats, T-06.3.4-25 (judge-spend meter fails open on a usage-absent provider response) and T-06.3.4-42 (judged-candidate selection missing the judgeable predicate the loop applies — a regression activated when plan 09 wired that path into production); `threats_open: 0`. A same-day code review then found and fixed a new interaction bug between plan 10's own two fixes (CR-01, plus warnings WR-01/WR-02/WR-03; see `06.3.4-REVIEW-FIX.md`), with 416/416 eval tests passing throughout. Re-verification (`06.3.4-VERIFICATION.md`, 2026-09-13T22:00:00Z) scored 6/8; SC-1 was then formally accepted as a disclosed deviation via a freshly-recorded `overrides:` entry (accepted interactively by the project owner 2026-09-13T22:20:00Z — superseding an earlier, procedurally-suspect override entry the same verification report identified and declined to trust), bringing the score to 7/8 (1 via override). SC-3 stays deferred (no calibration back-fill) — the sole remaining open gap, blocked on Phase 06.3.4.1/Phase 6.4.
...
- [x] 06.3.4-08-PLAN.md — `gap_closure: true`. Make the journal `partial` flag a function of the measured completeness comparison at every `drive()` exit path, make `score`/`report` fail closed when completeness cannot be positively verified, correct the live journal's header through the fixed reconcile, and correct the four records that assert otherwise — the journal-reset finding into `06.3.4-FINDINGS.md` §5, `06.3.4-04-SUMMARY.md`'s header claim, the dated SC-1 deviation amendment into `06.3.4-STAGED-GATE.md` §6, and `docs/evaluation-fidelity-and-graph-yield.md` point 5 (CR-01, T-06.3.4-19, WR-01, WR-05, IN-01; D-44, D-45)
