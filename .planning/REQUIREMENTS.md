...
- [ ] **DATA-04**: Extract entities and relationships during ingestion and persist them as graph nodes/edges in LanceDB. **SPIKE ONLY — full extraction pipeline deferred to Phase 04.1.**
- [ ] **DATA-05**: Query graph context with `lance-graph`/Cypher-style pattern matching and compile it into RAG prompt context. **SPIKE ONLY — Phase 04 empirically proved the lance-graph/lancedb integration pattern (see 04-RESEARCH.md, 04-AI-SPEC.md Status: CONFIRMED); full query/RAG-compile path deferred to Phase 04.1.**
