...
**`/gsd-capture --note <text>`** — Zero-friction timestamped note to `.planning/notes/` (or `D:/Repos/lancet/.cursor/notes/` globally). Subcommands: append (default), list, promote (note → todo). Works without a project.
