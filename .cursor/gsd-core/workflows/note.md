...
- **Global scope**: `D:/Repos/lancet/.cursor/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Repos/lancet/.cursor/notes/`)
...
2. Glob `D:/Repos/lancet/.cursor/notes/*.md` (if directory exists) — global notes
...
Global (D:/Repos/lancet/.cursor/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Repos/lancet/.cursor/notes/` — works in any directory
