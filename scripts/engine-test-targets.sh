...
if [ "$TOTAL" -ne 462 ]; then
  echo "FAIL: TOTAL test count mismatch: expected 462, got $TOTAL" >&2
  exit 1
fi

if [ "$LIB_BIN_SUM" -ne 422 ]; then
  echo "FAIL: lib + bin test count mismatch: expected 422, got $LIB_BIN_SUM (lib=$LIB_COUNT, bin=$BIN_MAIN_COUNT)" >&2
  exit 1
fi

if [ "$LIB_COUNT" -ne 422 ]; then
  echo "FAIL: engine (lib) test count mismatch: expected 422, got $LIB_COUNT" >&2
  exit 1
fi
