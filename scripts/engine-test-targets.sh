...
if [ "$TOTAL" -ne 486 ]; then
  echo "FAIL: TOTAL test count mismatch: expected 486, got $TOTAL" >&2
  exit 1
fi

if [ "$LIB_BIN_SUM" -ne 446 ]; then
  echo "FAIL: lib + bin test count mismatch: expected 446, got $LIB_BIN_SUM (lib=$LIB_COUNT, bin=$BIN_MAIN_COUNT)" >&2
  exit 1
fi

if [ "$LIB_COUNT" -ne 446 ]; then
  echo "FAIL: engine (lib) test count mismatch: expected 446, got $LIB_COUNT" >&2
  exit 1
fi
