...
if [ "$TOTAL" -ne 380 ]; then
  echo "FAIL: TOTAL test count mismatch: expected 380, got $TOTAL" >&2
  exit 1
fi

if [ "$LIB_BIN_SUM" -ne 345 ]; then
  echo "FAIL: lib + bin test count mismatch: expected 345, got $LIB_BIN_SUM (lib=$LIB_COUNT, bin=$BIN_MAIN_COUNT)" >&2
  exit 1
fi

if [ "$LIB_COUNT" -ne 345 ]; then
  echo "FAIL: engine (lib) test count mismatch: expected 345, got $LIB_COUNT" >&2
  exit 1
fi
